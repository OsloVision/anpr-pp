...
class NorwegianVehicleAPIClient:
    ...
    def get_vehicle_data(
        ...
        if not understellsnummer and not kjennemerke:
            raise VehicleAPIError("Either understellsnummer or kjennemerke must be provided")
        
        if understellsnummer and kjennemerke:
            raise VehicleAPIError("Only one of understellsnummer or kjennemerke should be provided")
    ...
    def debug_config(self) -> Dict[str, Any]:
        ...
        if 'SVV-Authorization' in config['headers']:
            ...
            if len(auth_header) > 20:
                config['headers']['SVV-Authorization'] = f"{auth_header[:15]}...{auth_header[-4:]}"
