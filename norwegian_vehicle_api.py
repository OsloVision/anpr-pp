...
class NorwegianVehicleAPI:
    ...
    def lookup_by_license_plate(self, license_plate: str) -> VehicleInfo:
        ...
        try:
            ...
            self.logger.error(f"Failed to lookup vehicle by license plate {license_plate}: {str(e)}")
            raise
    ...
    def _extract_vehicle_details(self, vehicle_data: EnkeltOppslagKjoretoydata, vehicle_info: VehicleInfo):
        """
        ...
        if vehicle_data.forstegangsregistrering:
            if vehicle_data.forstegangsregistrering.registrertForstegangNorgeDato:
                vehicle_info.registrert_forstgang = str(vehicle_data.forstegangsregistrering.registrertForstegangNorgeDato)
        
        # Inspection information
        if vehicle_data.periodiskKjoretoyKontroll:
            if vehicle_data.periodiskKjoretoyKontroll.kontrollfrist:
                vehicle_info.kontrollfrist = str(vehicle_data.periodiskKjoretoyKontroll.kontrollfrist)
    ...
    def get_raw_data(self, license_plate: Optional[str] = None, vin: Optional[str] = None) -> Dict[str, Any]:
        """
